...
public class TranslateVisitor implements codegen.C.Visitor
{
  ...
  private java.util.LinkedList<Block.T> cookBlocks()
  {
    ...
    while (i < size) {
      ...
      //System.out.println("dddddddddddddddddddddddd  " + label);
      while (i < size && this.stmOrTransfer.get(i) instanceof Stm.T) {
    	  stms.add((Stm.T) this.stmOrTransfer.get(i++));
        
      }
      //System.out.println("dddddddddddddddddddddddd  " + this.stmOrTransfer.get(i));
  ...
  public void visit(codegen.C.Ast.Exp.ArraySelect e)
  {
	  ...
	  //emit(new ArraySelect(array, index));
	  this.operand = new ArraySelect(array, index);
  ...
  public void visit(codegen.C.Ast.Exp.NewIntArray e)
  {
	  //TODO
	  e.exp.accept(this);
	  emit(new NewIntArray(e.name, this.operand));
...
//	  return;
	  e.exp.accept(this);
	  //emit(new Not(this.operand));
	  this.operand = new Not(this.operand);
  ...
  public void visit(codegen.C.Ast.Stm.AssignArray s)
  {
	  ...
	  Operand.T index = this.operand;
	  s.exp.accept(this);
	  Operand.T src = this.operand;
	  emit(new MoveArray(s.id, index, src));
  ...
  {
	  //TODO
//	  LinkedList<codegen.C.Ast.Stm.T> stms = new LinkedList<codegen.C.Ast.Stm.T>();
...
//		this.stm = new Block(stms);
  ...
  {
	  //TODO
	  util.Label tl = new util.Label(), fl = new util.Label(), ifff = new util.Label();
